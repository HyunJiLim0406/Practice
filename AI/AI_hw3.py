...
# 데이터 셋 만들기 ex)aaabbb->aabbb, aaaaabbbbb->aaaabbbbb
...
def tanh(x, derivative=False):
    # your job
    x_safe = x + 1e-12
    y = (np.exp(x_safe) - np.exp(-x_safe)) / (np.exp(x_safe) + np.exp(-x_safe))
...
# derivative : True 값이면 derivative를 리턴하고 아니면 forward pass를 리턴
def softmax(x, derivative=False):
    # your job
    x_safe = x + 1e-12
    y = np.exp(x_safe) / np.sum(np.exp(x_safe))
    if derivative:
        pass
    else:
        return y
...
def forward(inputs, hidden_state, params):
    ...
    for t in range(len(inputs.T)):
        hidden_state = tanh(np.dot(Wxh, inputs.T[t].reshape(vocab_size, 1)) + np.dot(Whh, hidden_state) + bh)
